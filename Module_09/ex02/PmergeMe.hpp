...
/*   Updated: 2026/06/18 20:18:38 by pabmart2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
private:
	...
	template <typename Container>
	void print_container(const Container &cont, std::size_t limit);
...
/**
 * @brief Parses a whitespace‑separated string of positive integers and loads
 *        them into a container.
 *
 * Splits the input string into tokens, validates each token as a positive
 * integer using `parse_positive_int`, and appends the resulting values to
 * the container. If no valid numbers are found, an exception is thrown.
 *
 * @tparam Container  Any container type supporting `push_back`.
 * @param input       String containing whitespace‑separated integer tokens.
 * @param container   Container where parsed values will be stored.
 *
 * @throws std::runtime_error If no numbers are successfully parsed.
 */
template <typename Container>
...
/**
 * @brief Checks whether a container is sorted in non‑decreasing order.
 *
 * Iterates through the container comparing each element with its successor.
 * If any element is greater than the next one, the function returns false.
 * Containers of size 0 or 1 are considered sorted.
 *
 * @tparam Container  Any iterable container type supporting forward iteration.
 * @param src         The container to check.
 *
...
/**
 * @brief Prints up to a fixed number of elements from a container.
 *
 * Iterates through the container and prints at most `limit` elements,
 * separating them with spaces. If the container contains more elements
 * than the specified limit, the output is truncated and the string
 * " [...]" is appended to indicate omitted values. A newline is printed
 * at the end.
 *
 * @tparam Container  Any iterable container type.
 * @param cont        The container whose elements will be printed.
 * @param limit       Maximum number of elements to print before truncation.
 */
template <typename Container>
void PmergeMe::print_container(const Container &cont, std::size_t limit)
{
	std::size_t count = 0;
	bool first = true;

	for (typename Container::const_iterator it = cont.begin();
		 it != cont.end() && count < limit;
		 ++it, ++count)
	{
		if (!first)
			std::cout << ' ';
...
/**
 * @brief Reorders a container according to the order specified by an iterator
 *        array.
 *
 * Copies the elements of the container into a temporary container following the
 * order defined by `it_array`, then replaces the original container with the
 * reordered one. Finally, the dynamically allocated iterator array is freed.
 *
 * @tparam Container  Type of the container being reordered.
 * @param cont        Container whose elements will be rearranged.
 * @param it_array    Dynamically allocated array of iterators defining the new
 *                    order.
 */
template <typename Container>
...
namespace ford_johson
{
	...
	/**
	 *  Core data structures used throughout the Ford–Johnson algorithm.
	 *
	 * Contains:
	 *
	 * - `s_Block` — Describes a MAIN or PEND block, including its iterators,
	 *               key value, logical index, and pairing information.
	 * - `s_SequenceContainer` — Groups the MAIN and PEND block arrays produced
	 *                           during the block‑building stage.
	 * - `s_JacobsthalSeq` — Stores a precomputed sequence of Jacobsthal numbers
	 *                       used to schedule PEND insertions.
	 * - `FJState` — Represents the full state of a single insertion phase,
	 *               including buffers, block metadata, counters, and Jacobsthal
	 *               data.
	 *
	 * This namespace centralizes all structural types shared across the
	 * pairing, block construction, insertion, and utility phases of the
	 * algorithm.
	 */
	namespace structs
	{
	...
	/**
	 * Low‑level utility functions used across the Ford–Johnson algorithm.
	 *
	 * Contains:
	 * -`build_jacobsthal_seq` — Generates a Jacobsthal sequence used to schedule
	 *                             PEND block insertions.
	 * -`binary_search_block_pos` — Performs a block‑boundary binary search to
	 *                               determine the correct insertion position for
	 *                               a PEND block.
	 * -`shift_right` — Safely shifts a contiguous range of elements to the right
	 *                   inside a buffer, preserving data during block insertion.
	 *
	 * These utilities support the pairing, block construction, and insertion
	 * phases by providing essential low‑level operations.
	 */
	namespace utils
	{
		/**
	...
	/**
	 * Implements the initial block‑pair ordering stage of the Ford–Johnson algorithm.
	 *
	 * Contains:
	 * - `swap_blocks` — Swaps two adjacent blocks of equal size inside the iterator array.
	 * - `ford_johnson_pairs` — Recursively orders block pairs by comparing their last
	 *                          elements and doubling the block size at each step.
	 *
	 * This namespace performs the first phase of the algorithm, ensuring that
	 * adjacent blocks are correctly ordered before MAIN/PEND construction and
	 * the insertion stages begin.
	 */
	namespace pairing
	{
		/**
	...
	/**
	 * Builds and manages MAIN/PEND block structures for the Ford–Johnson algorithm.
	 *
	 * Contains:
	 * - `build_sequences` — Partitions the iterator array into fixed‑size blocks and
	 *                       assigns them to the MAIN or PEND chains.
	 * - `build_insertion_buffer` — Constructs the working buffer used during the
	 *                              insertion phase, reserving space for PEND blocks.
	 * - `init_fj_state` — Initializes the insertion‑phase state, including MAIN/PEND
	 *                     blocks, buffer allocation, counters, and Jacobsthal data.
	 *
	 * This namespace implements the block‑construction stage of the algorithm and
	 * prepares all data required for the recursive insertion phase.
	 */
	namespace blocks
	{
		 ...
		 */
		template <typename Iterator>
		structs::s_SequenceContainer<Iterator>
		build_sequences(Iterator *it_array, std::size_t size, std::size_t block_size)
		{
			structs::s_SequenceContainer<Iterator> container;
			...
			container.main = new structs::s_Block<Iterator>[main_blocks];
			container.main_size = main_blocks;

			container.pend = new structs::s_Block<Iterator>[pend_blocks];
		...
		Iterator *build_insertion_buffer(Iterator *it_array,
										 const structs::s_SequenceContainer<Iterator> &container,
		 ...
		 */
		template <typename Iterator>
		structs::FJState<Iterator> init_fj_state(Iterator *it_array, std::size_t size,
												 std::size_t block_size)
		{
			structs::FJState<Iterator> st;

			st.it_array = it_array;
			st.size = size;
			st.block_size = block_size;
			st.container = blocks::build_sequences(it_array, size, block_size);
			...
			st.js = utils::build_jacobsthal_seq(st.container.pend_size);
			return st;
		}
	...
	/**
	 * Executes the Ford–Johnson block‑insertion phase.
	 *
	 * Contains:
	 * - `insert_block_into_main` — Inserts a single PEND block into the MAIN buffer
	 *                               while preserving global order.
	 * - `process_jacobsthal_blocks` — Inserts PEND blocks following the staged
	 *                                 Jacobsthal schedule.
	 * - `process_remaining_blocks` — Inserts any PEND blocks not covered by the
	 *                                Jacobsthal sequence.
	 * - `ford_johson_insertion` — Recursively performs the full insertion phase,
	 *                             halving the block size at each level.
	 *
	 * This namespace implements the core merging logic of the algorithm, combining
	 * MAIN and PEND blocks into a fully ordered sequence through staged insertion
	...
	namespace insertion
	{
		 ...
		 */
		template <typename Iterator>
		void insert_block_into_main(structs::FJState<Iterator> &st, std::size_t pend_block_index)
		{
			// Safety Checks
			if (pend_block_index >= st.container.pend_size)
				return;

			structs::s_Block<Iterator> &small = st.container.pend[pend_block_index];
			...
			// Aliases
			structs::s_SequenceContainer<Iterator> &container = st.container;
			...
			std::size_t target_pos =
				(upper == 0)
					? 0
					: utils::binary_search_block_pos(key, buffer, 0, upper, block_size);

			std::size_t from_index = target_pos * block_size;
			std::size_t count = st.inserted_blocks * block_size - from_index;
			utils::shift_right(buffer, from_index, count, block_size, st.size);
		 ...
		 */
		template <typename Iterator>
		void process_jacobsthal_blocks(structs::FJState<Iterator> &fj_state)
		{
			structs::s_SequenceContainer<Iterator> &container = fj_state.container;
			const structs::s_JacobsthalSeq &js = fj_state.js;
			...
			for (std::size_t i = 0; i < js.size; ++i)
			{
				...
				for (std::size_t j = jacobsthal_limit; j-- > last;)
					insertion::insert_block_into_main(fj_state, j);
		 ...
		 */
		template <typename Iterator>
		void process_remaining_blocks(structs::FJState<Iterator> &fj_state)
		{
			structs::s_SequenceContainer<Iterator> &container = fj_state.container;
			...
			for (std::size_t i = inserted_pend; i < container.pend_size; ++i)
				insertion::insert_block_into_main(fj_state, i);
		}
										...
										std::size_t size)
		{
			if (block_size == 0 || block_size >= size)
				return it_array;
			structs::FJState<Iterator> fj_state = blocks::init_fj_state(it_array, size, block_size);
	...
	typename Container::iterator *ford_johnson(Container &cont)
	{
		...
		iterator *it_array =
			pairing::ford_johnson_pairs(get_iterator_array(cont, size), 1, size);
		std::size_t block = 1;
		while (block * 2 <= size)
			block *= 2;
		...
		it_array = insertion::ford_johson_insertion(it_array, block, size);

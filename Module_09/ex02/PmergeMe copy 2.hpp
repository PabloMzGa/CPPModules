...
/*   Updated: 2026/06/15 19:12:15 by pablo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	Iterator block;       // Iterador real al elemento
                                            ...
                                            std::size_t block_size, size_t size)
{
	...
	info.main = new s_Block<Iterator>[size]();
	info.pend = new s_Block<Iterator>[size]();
	...
	{
		std::size_t block_type = i / block_size;
		std::size_t current_block_size = std::min(block_size, size - i);

		bool goes_to_main = (block_type == 0 || block_type % 2 == 1);
		bool full_block = (current_block_size == block_size);

		// Incomplete blocks must go to pend
		if (!full_block && block_type != 0)
			goes_to_main = false;

		s_Block<Iterator> block;
		block.block = new Iterator[current_block_size];
		for (size_t j = 0; j < current_block_size; j++)
		{
			if (goes_to_main)
			{
				info.main[main_i]->block[(main_i * block_size) + j] =
				    it_array[i + j];
				block.block_index = i;
				block.block_value = it_array[i + block_size];
				block.is_big = i != 0;
				++info.main_size;
			}
			else
...
{
	unsigned int key =
	    *info.pend[pend_block_index * block_size + (block_size - 1)];
